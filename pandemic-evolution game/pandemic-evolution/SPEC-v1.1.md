...
> **Nota v1.1:** a modelação económica profunda (cenários, simulação de volumes, fees definitivas, emissão de assets) foi **explicitamente adiada** para a fase de modelação económica. Esta versão resolve as contradições internas da v1.0 e fecha as decisões de design necessárias para iniciar a Phase 1.
...
## 26.1 [v1.1 — C5/D5] RECEITA ELEGÍVEL (LISTA PROVISÓRIA)

**Elegível:**

* compras primárias efetuadas por B (cosmetics, agents, passes, personalização);
* fees de marketplace pagas por B (quando B vende).

**Não elegível:**

* prémios de qualquer natureza;
* entradas de torneio (pass-through — §33);
* transferências entre jogadores;
* ganhos gerados por outros referrals;
* qualquer receita de atividades marcadas como suspeitas (risk scoring).

> Esta lista é **provisória** e será revalidada na fase de modelação económica. Janela de atribuição: vitalícia, 1 nível (revisitável).
...
**[v1.1 — C6]** Modelo inicial para modelação. A linha "Referral" é condicional: quando não existe referenciador válido, o valor segue para o Treasury (§27).

| Destino                          |        % |
| -------------------------------- | -------: |
| Referral **or Treasury**         |       5% |
...
**Âmbito:** esta tabela aplica-se a **receita própria** (compras primárias, fees de marketplace, fees de organização de torneios). **Não** se aplica ao pass-through de entradas de torneios (§33).
...
**Gate [v1.1]:** decisão de custódia (D6) validada juridicamente; devnet obrigatório (§47).
...
* revalidação da lista de receita elegível de referral (D5);
...
* decisão sobre USDC em prémios de season (D12);
* janela do referral (vitalícia vs. limitada).
...
*Fim da Master Specification v1.1.*
